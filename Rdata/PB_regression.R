...
# ── Packages ──────────────────────────────────────────────────────────────────
for (pkg in c("compiler", "boot", "ggplot2", "plotly", "htmlwidgets")) {
...
# ── Passing-Bablok algorithm (Passing & Bablok 1983) ─────────────────────────
PB.reg <- function(data, alpha, indices) {
  ...
  S      <- sort(S[seq_len(k)])
  N      <- length(S)
  K      <- floor(length(S[S < 0]) / 2)
  b      <- if (N %% 2 == 1) S[(N + 1) / 2 + K]
            else sqrt(S[N/2 + K] * S[N/2 + K + 1])
  a      <- median(y - b * x)
  Cg     <- qnorm(1 - alpha/2) * sqrt(lx*(lx-1)*(2*lx+5)/18)
  M1     <- round((N - Cg) / 2)
  M2     <- N - M1 + 1
  b.lo   <- S[M1 + K]; b.hi <- S[M2 + K]
  a.lo   <- median(y - b.hi * x); a.hi <- median(y - b.lo * x)
  c(a, b, a.lo, a.hi, b.lo, b.hi)
}
PB.reg.comp <- cmpfun(PB.reg)

PB.boot <- function(data, R, n.fit, alpha, xmin, xmax) {
...
plot.graph <- function(data, R, n.fit, alpha, pp) {
  reg    <- PB.reg.comp(data, alpha)
  a      <- reg[1]; b <- reg[2]
  x      <- data[, 1]; y <- data[, 2]
  resid  <- y - (a + b * x)

  boot.r <- PB.boot(data, R, n.fit, alpha, pp$xmin, pp$xmax)
  ci     <- boot.r$reg.CI.data
  reg.list <- list(intercept = a, slope = b,
                   CI.intercept = reg[3:4], CI.slope = reg[5:6], resid = resid)
...
rse     <- round(sqrt(sum(reg$resid^2) / (n - 2)), 3)
...
# Re-run bootstrap for CI data frame (numeric matrix now)
boot2  <- PB.boot(my.data[,1:2], R=200, n.fit=50, alpha=alpha,
                  xmin=pp$xmin, xmax=pp$xmax)
